...
To give a more structured example, consider a single agent having to move to
...
problems, two styles of solution will be built and tested.

First, a Q-Learning approach will be taken. This approach has been tested across
many different games, and should prove very effective at this task. If combined
...
paper~\cite{vinyals2017starcraft}, so there is proven results in the area. This
approach should be more global, since it takes input of the entire visual space,
and perhaps additional other information, to learn its environment sufficiently.
However, it is also possible that due to the higher complexity in being forced
to select more specific actions instead of the Q-learning approaches compound
actions, it may fair worse for tasks that require large amounts of tasks that
take multiple steps.
...
learning styles, and how each one fairs for a given challenge.
