...
Transfer learning is a process in machine learning that describes the practise
...
to be useful. A common example that is used to describe this process is that a
...
This can be useful for many reasons, the most obvious of which is lowering the
complexity of the new task, due to the network having existing knowledge. This
works best when the task at hand has not significantly changed, but the domain
has. A good example of this is the use of transfer learning in medical
applications, where knowledge in one domain can be very easily applied to
another\cite{van2015transfer}. However, what is perhaps less obvious but more
...
ImageNet dataset\cite{ILSVRC15}, for use in completely different domains
such as medical imaging\cite{shin2016deep, tajbakhsh2016convolutional}. This is
possible due to task at hand, recognising some feature or object, has stayed the
same despite the vast different in domain.

A more formal definition of Transfer learning is as follows, as taken from ``A
...
some pair $x_i \in X$ an $y_i \in \mathcal{Y}$. These labels are domain
specific, so for the MNIST dataset, they would be the number zero through nine.
...
This leads to 4 scenarios of transfer learning, where each of the 4 elements
that make up the $\mathcal{D}$ and $\mathcal{T}$ tuples are different.
