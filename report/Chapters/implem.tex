...
%TODO: Make a model of the architecture. Probably useful for the DQN as well.
...
\begin{enumerate}
    \item This following process is applied to both the mini-map and the screen
        ...
        mini-map.
        \begin{enumerate}
            \item Take input of the screen or mini-map (usually $32 \times 32$
                array), relative to the player, to a convolutional layer. Before
                inputting, the input is `one hot encoded' to swap it from being
                in the range 0--5 to in binary form.  From here, this
                information is combined with the unit types list (for the
                screen) and the non-player relative screen information (for
                both). All of this is passed to a single convolutional layer
                with 16 filters, using a kernel of size $(5 \times 5)$, a ReLU
                activation function, and a stride of 1. Padding is applied such
                that the input is the same size as the output.
    ...
    \item The two convolutional layers from the screen and mini-map are then
        ...
        split again to provide input to two more layers.
    \begin{enumerate}
        ...
        \item The second of these layers is a fully-connected layer. This takes
            ...
            unavailable actions have a probability of zero, before fixing the
            remaining values to sum to 1 again.
    \end{enumerate}
    ...
    \item Both of the spatial action and action id probabilities are
