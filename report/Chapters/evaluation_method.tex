...
policies. This makes them ideal to test a model again, such that the scores can
...
action commands. The end goal is to defeat the AI player, and win the game.
...
attack bases and units, search and take resources, and build its own units and
...
for measuring how the agent behaves generally, as it is possible for an agent to
excel at each of the underlying tasks that go into a game of StarCraft II, but
to fail in the process of combining them effectively.
