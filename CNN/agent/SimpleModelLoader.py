...
class SimpleModelLoader:
    ...
    def __init__(self, model_meta_path, graph_to_use, new_model_name):
        ...
        with self.graph.as_default():
            ...
            saver.restore(self.session, tf.train.latest_checkpoint(model_folder))
    ...
    def get_all_tensors_by_name(self, tensor_name_regex):
        ...
        for tensor_name in current_tensors:
            if (
                re.findall(tensor_name_regex, tensor_name)
                and not self.new_model_name in tensor_name
            ):

                print(f"Grabbing tensor: {tensor_name}")
                count += 1

                tensors.append(self.graph.get_tensor_by_name(f"{tensor_name}:0"))
