...
def pad_and_tile_non_spatial(conv_policy, log_non_spatial_features):
    ...
    non_spatial_dim_size = conv_policy.placeholders.non_spatial_features.get_shape().as_list()[
        1
    ]
    ...
    non_spatial_padded = tf.pad(log_non_spatial_features, padding, "CONSTANT")
    ...
    three_d_non_spatial = tf.tile(three_d_non_spatial_init, tiles)
    ...
    four_d_non_spatial = tf.expand_dims(three_d_non_spatial, 3)
...
def tile_and_tile_non_spatial(conv_policy, log_non_spatial_features):
    ...
    non_spatial_tiled_init = tf.tile(log_non_spatial_features, tiles)
...
def reference_tiling_method(conv_policy, log_non_spatial_features):
    ...
    three_d_non_spatial = tf.expand_dims(log_non_spatial_features, 1)

    if DEBUG:
        print(f"Three D shape: ({three_d_non_spatial.get_shape().as_list()})")

    four_d_non_spatial = tf.expand_dims(three_d_non_spatial, 2)

    if DEBUG:
        print(f"Four D shape: ({four_d_non_spatial.get_shape().as_list()})")

    tile_dims = tf.stack([1, conv_policy.spatial_dim, conv_policy.spatial_dim, 1])

    four_d_non_spatial_final = tf.tile(four_d_non_spatial, tile_dims)
