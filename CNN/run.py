...
flags.DEFINE_integer(
    "resolution", 32, "Resolution for screen and mini-map feature layers."
)
...
flags.DEFINE_integer(
    "K_batches", -1, "Number of training in thousands, -1 to run forever."
)
flags.DEFINE_integer(
...
flags.DEFINE_string(
    "model_name", "temp_testing", "Name for checkpoints and tensorboard summaries."
)
...
flags.DEFINE_string(
    "random_seed",
    None,
    "A seed to use for maps generated by PySC2 to reduce randomness",
)

flags.DEFINE_enum(
    "if_output_exists",
    "fail",
    ["fail", "overwrite", "continue"],
...
flags.DEFINE_float(
    "entropy_weight_spatial",
    1e-6,
    "Entropy of spatial action distribution loss weight.",
)
flags.DEFINE_float(
    "entropy_weight_action", 1e-6, "Entropy of action-id distribution loss weight."
)
...
if FLAGS.training:
    ...
    RELATIVE_SUMMARY_PATH = os.path.join(
        FLAGS.summary_path, "no_training", FLAGS.model_name
    )
...
def main():
    ...
    agent_details = sc2_env.AgentInterfaceFormat(
        feature_dimensions=sc2_env.Dimensions(
            screen=(FLAGS.resolution,) * 2, minimap=(FLAGS.resolution,) * 2
        )
    )
